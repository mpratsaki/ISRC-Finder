...
def _fill_track_block(block_nodes: list[Any], track: Mapping[str, Any], display_number: int) -> None:
    ...
    for p in paragraphs:
        if "Track 1" in p.text:
            ...
            _replace_first_across_runs(p, "To Party", title, required=False)
            _replace_first_across_runs(p, "00:02:36", format_duration_docx(_duration_ms(track)), required=False)
            _replace_first_across_runs(p, "00:00:00", format_duration_docx(_duration_ms(track)), required=False)
            break
