...
Additionally, Spotify's February 2026 Web API changes affect this file in two
very concrete ways that are easy to miss because Spotify's own error messages
are misleading:
  1. "Get Artist's Albums" (GET /artists/{id}/albums) had its `limit` maximum
     lowered from 50 to 10 (default is now 5). Sending limit=50 now returns
     HTTP 400 "Invalid limit" - the parameter name in the error is correct,
     but the actual problem is only that 50 exceeds the new max of 10.
  2. "Get Several Tracks" (GET /tracks?ids=...) was REMOVED entirely. The only
     way to fetch a track's ISRC now is GET /tracks/{id}, one track at a time.
     There is no batch endpoint to fall back to, so scanning a large discography
     for ISRCs is now inherently many individual requests (slower, by design
     of Spotify's new restrictions - not a bug in this app).
Reference: https://developer.spotify.com/documentation/web-api/references/changes/february-2026
"""
...
# Spotify's "Get Artist's Albums" endpoint had its limit lowered to max 10 as
# part of the February 2026 Web API changes (it used to be 50). Sending a
# higher value returns HTTP 400 "Invalid limit".
ARTIST_ALBUMS_PAGE_LIMIT = 10

# "Get Album Tracks" (GET /albums/{id}/tracks) is unaffected by the Feb 2026
# changes and still allows up to 50.
ALBUM_TRACKS_PAGE_LIMIT = 50
    ...
    for attempt in range(retries):
        ...
        if not resp.ok:
            try:
                ...
                if err_msg:
                    raise RuntimeError(f"Spotify API Error ({resp.status_code}): {err_msg} - URL: {resp.url}")
            except ValueError:
                pass
            resp.raise_for_status()
...
def fetch_tracks_from_albums(token, album_ids, status_placeholder=None):
    ...
    for idx, aid in enumerate(valid_album_ids, 1):
        ...
        try:
            ...
            params = {"limit": ALBUM_TRACKS_PAGE_LIMIT}
            while url:
                data = _api_get(token, url, params=params)
...
def fetch_full_tracks(token, track_ids, status_placeholder=None):
    """
    Ανάκτηση πλήρων στοιχείων τραγουδιών (κυρίως για το ISRC).

    ΣΗΜΑΝΤΙΚΟ (Spotify Web API changes, Φεβρουάριος 2026): το batch endpoint
    "Get Several Tracks" (GET /v1/tracks?ids=...) ΚΑΤΑΡΓΗΘΗΚΕ εντελώς. Το μόνο
    endpoint που απομένει είναι το GET /v1/tracks/{id}, ένα τραγούδι τη φορά.
    Γι' αυτό εδώ κάνουμε ΜΙΑ κλήση ανά track_id — δεν υπάρχει πλέον κανένα
    batch endpoint σαν fallback. Αυτό κάνει τη σάρωση μεγάλων discographies
    πιο αργή, αλλά είναι επιβεβλημένο από τη Spotify, όχι bug του app.
    """
    tracks = []
    valid_track_ids = [tid for tid in track_ids if tid and isinstance(tid, str) and tid != "null"]
    total = len(valid_track_ids)

    for idx, tid in enumerate(valid_track_ids, 1):
        if status_placeholder and (idx == 1 or idx % 5 == 0 or idx == total):
            status_placeholder.info(f"Ανάκτηση δεδομένων τραγουδιών (ISRC)... {idx}/{total}")
        ...
        try:
            ...
            # Αν αποτύχει ένα μεμονωμένο track (π.χ. 403/404), το προσπερνάμε
            continue
...
if source_mode == "Playlist (Από τον λογαριασμό σας)":
    ...
    st.caption(
        "Θα σαρωθούν αυτόματα Albums, Singles & EPs, Compilations και Appears On. "
        "Επειδή η Spotify κατάργησε το batch endpoint ISRC τον Φεβρουάριο 2026, "
        "η ανάκτηση των ISRC γίνεται πλέον ένα τραγούδι τη φορά — για μεγάλες "
        "δισκογραφίες η σάρωση μπορεί να πάρει αρκετά λεπτά."
    )
    ...
    if st.button("Δημιουργία Excel ✔️", type="primary"):
        ...
        try:
            ...
            # 4. Φορτώνουμε τα πλήρη δεδομένα (ISRC) — πλέον ένα-ένα τραγούδι,
            #    αφού το batch endpoint καταργήθηκε τον Φεβρουάριο 2026.
